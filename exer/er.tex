...
\iffalse\begin{minipage}{\textwidth}
...
There was some confusion about how to define a variation for a given submanifold. It's clear that we must use the exponential map:
\begin{align*}
	f: (-\varepsilon,\varepsilon)\times M &\longrightarrow \tilde{M} \\
	f(s,x) &=\operatorname{exp}_{p}(s N)
\end{align*}
where \(N\) is a normal (unit) vector at \(p \in M \subset \tilde{M}\).

Let \(M \subset (\tilde{M},g)\) be a submanifold. Define a variation \(f:(-\varepsilon,\varepsilon)\times M \to \tilde{M}\). Then for every \(s \in (-\varepsilon,\varepsilon)\), \(f_s\) is an immersion (embedding?) and we can pull back the metric of \(\tilde{M}\) to \(M_s\), i.e. \(f_s ^*g\). We have the volume functional 
\[S(s):= \int_{M_s}\operatorname{Vol}_{f_s ^*g}.\]
How to express volume in any coordinate system?
\[\sqrt{\det (f^* _sg)_{ij}}dx^1\wedge\ldots\wedge dx^n\]
And then you just differentiate this with respect to \(s\). You realise that you have to differentiate the square root of a determinant, which is a trace. Then you get to a mean curvature vector, which is also a trace. And that's the exercise. But you have to do it.
...
\subsection{weinstein theorem}

\begin{thing6}{autoexercício }\label{exer:}\leavevmode
Escreva a prova do teorema de Weinstein para entende la.
\end{thing6}
